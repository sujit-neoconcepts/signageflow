<template>
  <Transition>
    <fieldset ref="tableFieldset" :key="`table-${name}`" :dusk="`table-${name}`" class="min-w-0"
      ...
      <div :class="{ 'sticky top-10 z-20': stickyHeader }">
        <div class="flex flex-row flex-wrap sm:flex-nowrap justify-end px-0 py-1 pr-2 ">
          <div class="order-2 sm:order-3 mx-2 pt-1" v-if="queryBuilderProps.hasFilters && !props.hideFiltersColumn">
          ...
          <div class="flex flex-row w-full sm:w-auto sm:flex-grow order-1 sm:order-1 mb-2 sm:mb-0 ml-2 sm:mx-2 pt-1">
            <slot name="tableGlobalSearch" :has-global-search="queryBuilderProps.globalSearch"
              ...
              <TableGlobalSearch v-if="queryBuilderProps.globalSearch" class="w-full lg:w-1/2"
                :label="queryBuilderProps.globalSearch.label" :value="queryBuilderProps.globalSearch.value"
                :on-change="changeGlobalSearchValue" />
            </slot>
            <div class="order-4 mx-2  sm:order-6 " v-if="props.popupSearch">
              <div class="relative">
                <button type="button" title="Filter and Search"
          ...
          <slot name="tableAddSearchRow" :has-search-inputs="queryBuilderProps.hasSearchInputs"
            ...
            <TableAddSearchRow v-if="queryBuilderProps.hasSearchInputs && !props.hideSearchColumn"
          ...
          <div class="order-4 mx-2  sm:order-5 pt-1" v-if="bulkDelete">
            <div class="relative">
              <button ref="button" type="button" title="Delete Selected" :disabled="!selectedRows.length"
                ...
                @click="isModalSelectedDeleteActive = true" :class="{
                  'bg-gray-200 dark:bg-slate-700 text-gray-400': !selectedRows.length,
                  'bg-red-600 dark:bg-red-500 text-gray-100': selectedRows.length,
                }">
          ...
          <div class="order-4 mx-2  sm:order-5 pt-1"
            v-if="props.resourceNeo.bulkActions && props.resourceNeo.bulkActions.csvExport">
            <div class="relative">
              <button ref="button" type="button" title="Export To csv" :disabled="!selectedRows.length"
                class="w-full border border-gray-200 dark:border-slate-700 text-opacity rounded-md shadow-sm px-4 py-2 inline-flex justify-center text-sm font-medium hover:border-gray-600 hover:dark:border-slate-100 border-gray-300"
                @click="exportTableToCSV()" :class="{
                  'bg-gray-200 dark:bg-slate-700 text-gray-400': !selectedRows.length,
                  'bg-gray-50 dark:bg-gray-50 text-black': selectedRows.length,
                }">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 " viewBox="0 0 20 20" fill="currentColor">
                  <path fill="currentColor"
          ...
          <div class="order-4 mx-2  sm:order-6 pt-1" v-if="props.advanceSort">
            <div class="relative">
              <button type="button" title="Advance Sort"
                class="w-full border border-gray-200 dark:border-slate-700 bg-gray-400 dark:bg-slate-500 text-opacity rounded-md shadow-sm px-4 py-2 inline-flex justify-center text-sm font-medium  hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                @click="isModalSortPopupActive = true">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-white-400" viewBox="0 0 22 22"
                  fill="currentColor">
                  <path
          ...
          <slot name="tableColumns" :has-columns="queryBuilderProps.hasToggleableColumns"
            ...
            <TableColumns v-if="queryBuilderProps.hasToggleableColumns && !props.hideToggleColumn"
              class="order-6 mx-2 sm:mr-0 sm:order-7 pt-1" :columns="queryBuilderColumns"
      ...
      <slot name="tableSearchRows" :has-search-rows-with-value="queryBuilderProps.hasSearchInputsWithValue"
        ...
        <TableSearchRows
          v-if="!props.hideSearchColumn && (queryBuilderProps.hasSearchInputsWithValue || forcedVisibleSearchInputs.length > 0)"
          :search-inputs="queryBuilderProps.searchInputsWithoutGlobal"
          :forced-visible-search-inputs="forcedVisibleSearchInputs" :on-change="changeSearchInputValue"
      ...
      <div v-if="props.resourceNeo.cellDetail" class="px-2  flex flex-row">
        ...
        <input type="text"
          ...
          v-model="detailCellRef" v-show="(true)" ref="dcellselectref" @keydown.enter.prevent="goto">
        <input type="text" ref="dcellselect"
          ...
          v-model="detailCell" readonly @keydown.ctrl.c="controlCopy()" @keydown.enter.prevent="entertoEdit"
          @keydown.up="gotoup" @keydown.down="gotodown" @keydown.left="gotoleft" @keydown.right="gotoright">
        <div class="ml-1" v-show="(true)">
          <button type="button" @click="hx"
            class="w-full border border-gray-200 dark:border-slate-700   rounded-md shadow-sm px-2 py-2  inline-flex justify-center text-sm font-medium hover:border-gray-600 hover:dark:border-slate-100 border-gray-300 bg-gray-200 dark:bg-slate-700 text-gray-400"
            title="Hx">
            <span>Hx</span>
          </button>
      ...
      <slot name="tableWrapper" :meta="resourceMeta">
        <TableWrapper :class="{ 'mt-1': !hasOnlyData }" :stickyHeader="stickyHeader">
          <slot name="table">
            <table id="datatable" cllass="border-t border-gray-500 mx-2 rounded" v-columns-resizable>
              <thead>
                <slot name="head" :show="show" :sort-by="sortBy" :header="header">
                  <tr>
                    <th v-if="!props.resourceNeo.DisCheckboxes"
                      :class="{ ['!' + headerColor + ' dark:!' + headerColor + ' text-white ']: headerColor, }"><input
                        type="checkbox" :checked="selectAll" @change="selectAllItems" />
                    </th>
                    <HeaderCell v-for="  column   in   queryBuilderProps.columns  " :colHeader="colHeader"
                      :key="`table-${name}-header-${column.key}`" :cell="header(column.key)"
                      :editIcon="column.extra.editable" @columnToggle="columnToggle($event)" :headerColor="headerColor" />
                  </tr>
              ...
              <tbody>
                <slot name="body" :show="show">
                  <tr v-for="(  item, key  ) in   resourceData  " :key="`table-${name}-row-${key}`"
                    class="border-b border-t" :class="{
                      'bg-gray-50': striped && key % 2, 'lg:!bg-gray-300 lg:dark:!bg-slate-400/70': selectedRow == `${item.id}`, '!bg-gray-400': selectedRows.includes(item.id)
                    }
                      ">
                    <td v-if="!props.resourceNeo.DisCheckboxes" :data-id="item.id"><input type="checkbox"
                        :checked="selectedRows.includes(item.id)" @click.prevent="selectItem(item)" /></td>
                    <td v-for="  column   in   queryBuilderProps.columns  " v-show="show(column.key)"
                      :key="`table-${name}-row-${key}-column-${column.key}`"
                      class="text-sm p-0 visited:outline outline-white outline-1 !bg-opacity-75" ref="refColumn"
                      :dataid="item.id" :dataval="item[column.key]" :datakey="column.key"
                      @dblclick.revent="showpopper(item.id, column.key)"
                      @click="detailCellShow(item.id, column.key, column.extra.colKey); focustoDisplayCell();"
                      :class="{ 'bg-slate-300 dark:bg-gray-700 ': column.extra.hidden && !column.extra.bg, [column.extra.bg + ' dark:' + column.extra.bg + ' text-black ']: column.extra.bg, '!bg-stone-400 dark:!bg-stone-400 !border-2 !border-purple-800': selectedCell == `${item.id}-${column.key}` }"
                      :colKey="column.extra.colKey" :id="`${column.extra.colKey}${item.id}`"
                      :dispval="column.extra.dispval"
                      v-bind:style="{ 'max-width': column.extra.width, 'min-width': column.extra.width }">
                      <div class="max-h-16 overflow-y-hidden overflow-x-hidden p-1 break-words">
                        <slot :name="`cell(${column.key})`" :item="item" :skey="key">
                          {{ item[column.key] }}
                        </slot>
                      </div>
                    </td>
      ...
      <slot name="pagination" :on-click="visit" :has-data="hasData" :meta="resourceMeta"
        ...
        <Pagination :on-click="visit" :has-data="hasData" :meta="resourceMeta" :selectedRows="selectedRows"
          :per-page-options="queryBuilderProps.perPageOptions" :on-per-page-change="onPerPageChange" />
      </slot>
    </fieldset>
  </Transition>
  <CardBoxModal v-model="isModalSearchPopupActive" buttonLabel="Filter" title="Filter and Search" button="sucess"
    has-cancel :fullWidth="(true)">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-1">
      <TableWideSearch :filters="queryBuilderProps.searchInputsWithoutGlobal" :columns="queryBuilderProps.columns"
        v-model="searchFields" @formSubmit="doPopupSearch" />
    </div>

    <template #footer>
      <BaseButtons>
        <BaseButton label="Filter" color="success" @click="doPopupSearch" />
        <BaseButton label="Reset" color="warning" outline @click="resetSearchField" />
        <BaseButton label="Cancel" color="warning" outline @click="isModalSearchPopupActive = false" />
      </BaseButtons>
    </template>

  </CardBoxModal>

  <CardBoxModal v-model="isModalSortPopupActive" buttonLabel="Sort" title="Advance Sort" button="sucess" has-cancel>
    <div class="lg:max-h-96 overflow-y-auto">
      <AdvanceSort v-for="(selm, index) in advSortFields" :key="`sortkey${index}`" :index="index" :column="selm"
        @delrow="deleteSortRow" @sortorder="sortOrder" v-model="advSortFields[index]" :advSortFields="advSortFields"
        @sortfield="sortField" :allcolumns="queryBuilderProps.columns" />
    </div>
    <BaseButton label="Add another sort column" color="success" @click="addSortFields" />
    <template #footer>
      <BaseButtons>
        <BaseButton label="Sort" color="success" @click="doPopupSort" />
        <BaseButton label="Reset" color="warning" outline @click="resetSortField" />
        <BaseButton label="Cancel" color="warning" outline @click="isModalSortPopupActive = false" />
      </BaseButtons>
    </template>

  </CardBoxModal>

  <CardBoxModal v-model="isModalSelectedDeleteActive" buttonLabel="Confirm" title="Please confirm" button="danger"
    has-cancel @confirm="deleteSelected">
    <p>Are you sure to Delete Selected Item<span class="text-green-400">({{ selectedRows.length }})</span>?</p>
  </CardBoxModal>



  <CardBoxModal v-model="isEditPopupActive" buttonLabel="Update" :title="editPopupTitile" button="info" has-cancel
    @confirm="popupEditFunction" @cancel="gotowithotmessage">
    <input v-if="editPopupFieldType == 'input'" v-model="editPopupModel"
      @keydown.enter.prevent="isEditPopupActive = false; popupEditFunction()" ref="poperinput"
    ...
    <textarea v-if="editPopupFieldType == 'textarea'" v-model="editPopupModel" @keydown="inputHandlerTextarea"
      ref="poperinput"
      class="px-3 py-1 max-w-full focus:ring focus:outline-none border-gray-700 rounded w-full dark:placeholder-gray-400 h-24 border bg-white dark:bg-slate-800"></textarea>
    <div v-if="editPopupFieldType == 'textarea'">Press SHIFT+ENTER to enter a new line (line break) because pressing ENTER
      button alone will SAVE existing data (and not create line break).</div>
    <CheckboxMulti v-if="editPopupFieldType == 'checkbox'" v-model="editPopupModel" :options="editPopupModelOptions" />

    <FormControl v-if="editPopupFieldType == 'select'" v-model="editPopupModel" :options="editPopupModelOptions"
      ref="poperselect" @keydown.enter.prevent="isEditPopupActive = false; popupEditFunction()" />
    <VueDatePicker input-class-name="text-gray-500 dark:text-gray-100 shadow-sm text-sm bg-gray-100 dark:bg-slate-600"
      :month-change-on-scroll="false" :range="false" :enable-time-picker="false" format="yyyy-MM-dd" auto-apply
      v-if="editPopupFieldType === 'datePicker'" :model-value="editPopupModel" @update:model-value="setFilterValue">
    </VueDatePicker>

    <div
      v-if="editPopupFreeTextFieldType && (editPopupFieldType == 'select' || editPopupFieldType == 'checkbox' || editPopupFieldType === 'datePicker')">
      <div v-if="editFreeTextEnableModel" class="text-orange-600 text-md">Non-free text field(s) disabled. To re-enabled,
        please uncheck "free text" input to <b>RESET</b> input {{ editPopupFreeTextFieldType }}</div>
      <br><br>
      <textarea v-model="editFreeTextPopupModel" @keydown="inputHandlerTextarea" ref="poperinput"
        ...
        :class="{ 'bg-white dark:bg-slate-800': editFreeTextEnableModel, 'bg-gray-100 dark:bg-gray-300': !editFreeTextEnableModel }"
        :readonly="!editFreeTextEnableModel" v-if="editPopupFreeTextFieldType == 'textarea'"></textarea>
      <input v-if="editPopupFreeTextFieldType == 'input'" v-model="editFreeTextPopupModel"
        @keydown.enter.prevent="isEditPopupActive = false; popupEditFunction()" ref="poperinput"
      ...
      <div
        v-if="editPopupFreeTextFieldType && editFreeTextPopupModel && ((editPopupFreeTextFieldType == 'input' && editFreeTextPopupModel.length >= 255) || (editPopupFreeTextFieldType == 'textarea' && editFreeTextPopupModel.length >= 65000))"
        class="text-red-600"> Length Exceed!!</div>
      <input v-model="editFreeTextEnableModel" type="checkbox" @change="freeTextCheckHandler"
    ...
    <div
      v-if="editPopupModel && ((editPopupFieldType == 'input' && editPopupModel.length >= 255) || (editPopupFieldType == 'textarea' && editPopupModel.length >= 65000))"
      class="text-red-600"> Length Exceed!!</div>
  </CardBoxModal>
  <CardBoxModal v-model="isModalHxActive" title="Update History" has-cancel @confirm="isModalHxActive = false" full-width
    :hasFooter="(!1)" @cancel="gotowithotmessage">
    ...
    <div class="lg:max-h-96 overflow-y-auto">
      ...
      <div v-if="historydata.length == 0 && !isModalHxActiveLoader">No History Found</div>
      <CardBox class="mb-1 mr-1 p-2 last:mb-0 bg-white dark:!border-solid" hasTable="(!0)" is-hoverable
        v-for="hist in historydata " :key="`histkey${hist.id}`">
  ...
  <CardBoxModal v-model="isModelUncheckActive" buttonLabel="Delete" title="Please confirm" button="danger" has-cancel
    @confirm="uncheckConfirm" @cancel="uncheckCancel">
    <p>Are you sure you want to DELETE all current contents and revert to non-free-text field(s)? Lost contents cannot be
      recovered.</p>
  </CardBoxModal>
</template>
...
const hx = () => {
  ...
  if (el) {
    ...
    const historyReq = { module: props.resourceNeo.resourceName, did: did, dkey: dkey };
    axios.post(route("activitylog.history"), historyReq)
      .then(response => {
...
const dcellselectref = ref(null);
const controlCopy = () => {
  dcellselect.value.select();
  document.execCommand("copy");
  useToast().info('Copied to clipboard!!');
  dcellselectref.value.focus();
}

const bulkDelete = computed(() => {
  if (props.resourceNeo.bulkActions && props.resourceNeo.bulkActions.bulk_delete) {
    return true;
  }
});
...
function popupEditFunction() {
  ...
  if (editPopupFieldType.value == 'input' && editPopupModel.value && editPopupModel.value.length >= 255) {
    useToast().error('Text Length Excced!!');
    return false;
  }

  if (editFreeTextEnableModel.value == true && editPopupFreeTextFieldType.value == 'input' && editFreeTextPopupModel.value && editFreeTextPopupModel.value.length >= 255) {
    useToast().error('Text Length Excced!!');
    return false;
  }

  if (editFreeTextEnableModel.value == false && editPopupModel.value && editPopupModel.value.id == null && editPopupModel.value.label == '') {
    editPopupModel.value = null;
  }
  router.post(route(props.resourceNeo.resourceName + ".fieldUpdate",
    {
      id: refColumn.value[editPopupRefId.value].getAttribute("dataid"),
      key: refColumn.value[editPopupRefId.value].getAttribute("datakey"),
      ref: refColumn.value[editPopupRefId.value].getAttribute("id"),
      value: editFreeTextEnableModel.value == true ? editFreeTextPopupModel.value : (editPopupModel.value && editPopupModel.value.id ? editPopupModel.value.id : editPopupModel.value)
    }))
}
...
const inputHandlerTextarea = (e) => {
  if (e.keyCode === 13 && !e.shiftKey) {
    e.preventDefault();
    isEditPopupActive.value = false; popupEditFunction()
  }
}

function getRefElement(id, key) {
  if (refColumn.value) {
    let refid = 0;
    refColumn.value.forEach((elm, i) => {
      if (elm.getAttribute("dataid") == id && elm.getAttribute("datakey") == key) {
        refid = i;
      }
    });
    return refid;
...
function detailCellShow(id, key, colkey = 'A') {
  ...
  //const refid = getRefElement(id, key);
  ...
  if (el != null) {
    ...
    detailCell.value = el.getAttribute("dataval") != el.innerText ? el.innerText : el.getAttribute("dataval");
...
const gotoright = (e) => {
  ...
  if (el) {
    let cIndex = el.cellIndex;
    if (cIndex < document.getElementById('datatable').rows[0].cells.length - 1) {
...
const gotocell = (el) => {
  if (el) {
    if (el.style && el.style.display == 'none') {
      useToast().error('Cell refrence found but hidden!! Expand to see.');
    }
    ...
    selectedCell.value = el.getAttribute("dataid") + "-" + el.getAttribute("datakey");
    detailCellShow(el.getAttribute("dataid"), el.getAttribute("datakey"), el.getAttribute("colKey"));
    focustoDisplayCell();
  }
  else {
    useToast().error('Cell Ref NOT found in this Page. Pls Use Filter & Search instead!!');
    return false;
  }
}
...
function entertoEdit() {
  if (props.enterKeyToEdit && selectedCell.value) {
    let idkey = selectedCell.value.split("-");
    showpopper(idkey[0], idkey[1]);
  }
}
function showpopper(id, key) {
  const refid = getRefElement(id, key);
  if (refid != null) {
    editPopupRefId.value = refid;
    const columnIndex = findDataKey("columns", refColumn.value[refid].getAttribute("datakey"));
    if (!queryBuilderData.value.columns[columnIndex].extra.editable) {
      useToast().error('Not Editable!!');
      return false;
    }

    if (queryBuilderData.value.columns[columnIndex].extra.shoinfoonedit) {
      infotext.value = queryBuilderData.value.columns[columnIndex].extra.info
    }
    else {
      infotext.value = '';
    }
    editPopupTitile.value = 'Update ' + queryBuilderData.value.columns[columnIndex].label;
    editPopupModel.value = refColumn.value[refid].getAttribute("dataval");
    editFreeTextPopupModel.value = refColumn.value[refid].getAttribute("dataval");
    editFreeTextEnableModel.value = false;

    editPopupFieldType.value = queryBuilderData.value.columns[columnIndex].extra.type;

    editPopupFreeTextFieldType.value = queryBuilderData.value.columns[columnIndex].extra.freetext;

    if (typeof queryBuilderData.value.columns[columnIndex].extra.type == 'undefined') {
      editPopupFieldType.value = 'input'
    }

    else if (queryBuilderData.value.columns[columnIndex].extra.type == 'select') {
      editPopupFieldType.value = queryBuilderData.value.columns[columnIndex].extra.type;
      editPopupModelOptions.value = clone(queryBuilderData.value.columns[columnIndex].extra.options);
      editPopupModelOptions.value.shift();
      //editPopupModelOptions.value[0]['label'] = 'Select ' + editPopupModelOptions.value[0]['label'];
      editPopupModel.value = { 'id': refColumn.value[refid].getAttribute("dataval"), 'label': refColumn.value[refid].innerText };
      var indexedArray = [];
    ...
    else if (queryBuilderData.value.columns[columnIndex].extra.type == 'checkbox') {
      editPopupFieldType.value = queryBuilderData.value.columns[columnIndex].extra.type;
      editPopupModelOptions.value = queryBuilderData.value.columns[columnIndex].extra.options;
      let temparray = refColumn.value[refid].getAttribute("dataval") ? refColumn.value[refid].getAttribute("dataval").split(/\r?\n/) : [];
    ...
    if (typeof queryBuilderData.value.columns[columnIndex].extra.type == 'undefined' || queryBuilderData.value.columns[columnIndex].extra.type == 'input' || queryBuilderData.value.columns[columnIndex].extra.type == 'textarea') {
      setTimeout(function () { poperinput.value.focus() }, 100);
    }
    if (queryBuilderData.value.columns[columnIndex].extra.type == 'select') {
      setTimeout(function () {
...
const doPopupSearch = () => {
  ...
  let tempv;
  Object.entries(queryBuilderProps.value.searchInputsWithoutGlobal).forEach(element => {
    if (queryBuilderColumns.value[findDataKey("columns", element[1].key)].extra.type == 'datePicker') {
...
const deleteSelected = () => {
  router.delete(route(props.resourceNeo.resourceName + ".bulkDestroy", { ids: selectedRows.value }), {
    preserveScroll: true,
...
const queryBuilderColumns = ref(JSON.parse(JSON.stringify(queryBuilderProps.value.columns)));
...
function sortBy(column) {
  if (props.multipleSort) {
    const allsorts = queryBuilderData.value.sort ? queryBuilderData.value.sort.split(",") : [];
    var found = false;
    ...
    queryBuilderData.value.sort = allsorts.length ? allsorts.join(',') : column;
    if (props.advanceSort) {
      advSortFields.value = [];
      const allsorts2 = queryBuilderData.value.sort ? queryBuilderData.value.sort.split(",") : [];
...
function exportTableToCSV($event, excludelast) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = props.resourceNeo.bulkActions ? 1 : 0; j < collength; j++) {
...
function columnToggle(clickedcolumns) {
  clickedcolumns.forEach(function (col) {
    ...
    queryBuilderColumns.value[intKey].hidden = !queryBuilderColumns.value[intKey].hidden;

    if (queryBuilderColumns.value[intKey].extra.showhide2 && queryBuilderColumns.value[intKey].hidden) {
    ...
    setTimeout(function () {
      ...
      ths.forEach((th, index) => {
        if (th.style.width == '0px' && th.style.display != 'none') {
          table.style.width = (cutPx(table.style.width) + th.offsetWidth) + 'px';
...
function makeresizable(el) {
  ...
  ths.forEach((th, index) => {
    th.style.width = cutPx(th.style.width) > 0 ? th.style.width : th.offsetWidth + 'px';
  ...
  const handleResize = e => {
    if (moving) {
      var cnt = 0
      while (movingIndex > 0 && cutPx(ths[movingIndex + cnt].style.width) == 0) {
        cnt--;
      }
...
defineExpose({
  resetSelect, showHideAll
});

watch(selectedRows, () => {
  if (props.seleLimit > 0 && selectedRows.value.length > props.seleLimit) {
    selectedRows.value.pop();
    useToast().error('You can not select more than ' + props.seleLimit + ' record!!');
  }
}, { deep: true });
