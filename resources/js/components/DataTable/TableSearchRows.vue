  ...
  <div v-for="(searchInput, key) in searchInputs" v-show="searchInput.value !== null || isForcedVisible(searchInput.key)"
    ...
    <div class="flex rounded-md shadow-sm relative mt-3">
      <label :for="searchInput.key"
        class="inline-flex items-center px-4 rounded-l-md border border-gray-200 dark:border-slate-700 bg-gray-200 dark:bg-slate-700 text-opacity border-r-0  text-sm">
      ...
      <input :id="searchInput.key" :ref="skipUnwrap.el" :key="searchInput.key" :name="searchInput.key"
        ...
        class="flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-r-md  focus:border-transparent focus:ring-0 text-sm border border-gray-200 dark:border-slate-700 bg-gray-100 dark:bg-slate-600 text-opacity"
