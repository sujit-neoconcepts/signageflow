<script setup>
import { computed, ref, onMounted, onBeforeUnmount } from "vue";
import { useMainStore } from "@/stores/main";
import FormControlIcon from "@/components/FormControlIcon.vue";

import BaseIcon from "@/components/BaseIcon.vue";
import { mdiEyeOutline, mdiEyeOffOutline } from "@mdi/js";

const props = defineProps({
    name: {
        type: String,
        default: null,
    },
    id: {
        type: String,
        default: null,
    },
    autocomplete: {
        type: String,
        default: null,
    },
    maxlength: {
        type: String,
        default: null,
    },
    placeholder: {
        type: String,
        default: null,
    },
    inputmode: {
        type: String,
        default: null,
    },
    icon: {
        type: String,
        default: null,
    },
    options: {
        type: Array,
        default: null,
    },
    type: {
        type: String,
        default: "text",
    },
    modelValue: {
        type: [String, Number, Boolean, Array, Object],
        default: "",
    },
    required: Boolean,
    disabled: Boolean,
    readonly: Boolean,
    borderless: Boolean,
    transparent: Boolean,
    ctrlKFocus: Boolean,
    color: {
        type: String,
        default: null,
    },
    step: {
        type: [String, Number],
        default: null,
    },
});

const emit = defineEmits(["update:modelValue", "setRef", "focus", "blur"]);

const computedValue = computed({
    get: () => props.modelValue,
    set: (value) => {
        emit("update:modelValue", value);
    },
});

const inputElClass = computed(() => {
    const base = [
        "px-3 py-1 max-w-full focus:ring focus:outline-none border-gray-700 rounded w-full",
        "dark:placeholder-gray-400",
        "disabled:bg-gray-100 dark:disabled:bg-slate-700/50 disabled:text-gray-500 disabled:cursor-not-allowed",
        "read-only:bg-gray-100 dark:read-only:bg-slate-700/50 read-only:text-gray-500",
        computedType.value === "textarea" ? "h-24" : "h-10",
        props.borderless ? "border-0" : "border",
        props.color ? props.color : (props.transparent ? "bg-transparent" : "bg-white dark:bg-slate-800"),
    ];

    if (props.icon) {
        base.push("pl-10");
    }

    return base;
});

const computedType = computed(() =>
    props.options
        ? "select"
        : props.type == "password"
        ? showHidepassword.value
            ? "password"
            : "text"
        : props.type
);

const controlIconH = computed(() =>
    props.type === "textarea" ? "h-full" : "h-12"
);

const mainStore = useMainStore();

const selectEl = ref(null);

const textareaEl = ref(null);

const inputEl = ref(null);

onMounted(() => {
    if (selectEl.value) {
        emit("setRef", selectEl.value);
    } else if (textareaEl.value) {
        emit("setRef", textareaEl.value);
    } else {
        emit("setRef", inputEl.value);
    }
});

if (props.ctrlKFocus) {
    const fieldFocusHook = (e) => {
        if (e.ctrlKey && e.key === "k") {
            e.preventDefault();
            inputEl.value.focus();
        } else if (e.key === "Escape") {
            inputEl.value.blur();
        }
    };

    onMounted(() => {
        if (!mainStore.isFieldFocusRegistered) {
            window.addEventListener("keydown", fieldFocusHook);
            mainStore.isFieldFocusRegistered = true;
        } else {
            // console.error('Duplicate field focus event')
        }
    });

    onBeforeUnmount(() => {
        window.removeEventListener("keydown", fieldFocusHook);
        mainStore.isFieldFocusRegistered = false;
    });
}
const showHidepassword = ref(true);
const togglePassword = () => {
    showHidepassword.value = !showHidepassword.value;
};

const selectFocus = () => {
    selectEl.value.focus();
};
defineExpose({
    selectFocus,
});
</script>

<template>
    <div class="relative">
        <select
            v-if="computedType === 'select'"
            :id="id"
            v-model="computedValue"
            :name="name"
            :class="inputElClass"
            ref="selectEl"
        >
            <option
                v-for="option in options"
                :key="option.id ?? option"
                :value="option"
            >
                {{ option.label ?? option }}
            </option>
        </select>
        <textarea
            v-else-if="computedType === 'textarea'"
            :id="id"
            v-model="computedValue"
            :class="inputElClass"
            :name="name"
            :maxlength="maxlength"
            :placeholder="placeholder"
            :required="required"
        />
        <input
            v-else
            :id="id"
            ref="inputEl"
            v-model="computedValue"
            :name="name"
            :maxlength="maxlength"
            :inputmode="inputmode"
            :autocomplete="autocomplete"
            :required="required"
            :disabled="disabled"
            :readonly="readonly"
            :placeholder="placeholder"
            :type="computedType"
            :step="step"
            :class="inputElClass"
            @focus="emit('focus', $event.target.value)"
            @blur="emit('blur', $event.target.value)"
        />
        <FormControlIcon v-if="icon" :icon="icon" :h="controlIconH" />
        <div
            @click="togglePassword"
            v-if="props.type === 'password'"
            class="cursor-pointer justify-center items-center w-10 h-12 absolute top-1 right-0 text-gray-500 dark:text-slate-400"
        >
            <BaseIcon
                :path="showHidepassword ? mdiEyeOutline : mdiEyeOffOutline"
                w="w-10"
                h="h-10"
                size="24"
                class="absolute top-0 left-0 text-gray-500 dark:text-slate-400"
            />
        </div>
    </div>
</template>
